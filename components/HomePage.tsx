...
function Countdown() {
  ...
  useEffect(() => {
    const tick = () => {
      const diff = METALLIC.betaDate - Date.now();
      if (diff <= 0) {
...
export default function HomePage({ navigate }: HomePageProps) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      <div style={{ position: 'absolute', top: 'clamp(80px, 12vh, 130px)', left: 0, right: 0, zIndex: 5, display: 'flex', justifyContent: 'center' }}>
        <Countdown />
      </div>

      <div style={{ position: 'absolute', bottom: 'clamp(20px,4vh,48px)', left: 'clamp(16px,4vw,40px)', zIndex: 5, userSelect: 'none', pointerEvents: 'none', maxWidth: '55vw' }}>
        <p style={{
        ...
        <p style={{
          ...
          Private ecosystem of AI systems,<br />automation tools, and flagship products.
        </p>
      </div>

      <div style={{ position: 'absolute', bottom: 'clamp(20px,4vh,48px)', right: 'clamp(16px,4vw,40px)', zIndex: 5 }}>
        <button
          ...
          style={{
            ...
            letterSpacing: '0.2em',
            textTransform: 'uppercase',
            color: workshopHover ? C.green : C.white,
            textShadow: workshopHover ? `0 0 24px rgba(0,200,122,0.5), ${textShadow}` : textShadow,
